...
class MockBrokerAdapter(BrokerAdapter):
    ...
    async def submit_order(self, order: Order) -> str:
        ...
        if random.random() < self.rejection_rate:
            logger.warning(f"Mock broker rejecting order (random failure)")
    ...
    async def _update_position_from_order(self, order: Order):
        ...
        quantity_change = order.filled_quantity if order.direction == OrderDirection.BUY else -order.filled_quantity
        ...
        if not existing:
            ...
            if new_qty == 0:
                ...
                existing.average_price = (old_qty * old_avg + quantity_change * order.average_fill_price) / new_qty
    ...
    async def get_order_status(self, order_id: str) -> Order:
        ...
        if order.status == OrderStatus.FILLED and order.symbol in self.positions:
            ...
            pos.unrealized_pnl_percent = (pnl_diff / (pos.average_price * pos.quantity)) * 100 if pos.average_price != 0 else 0
