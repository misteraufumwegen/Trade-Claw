...
from app.routing.broker_session_router import BrokerType, BrokerSessionRouter
...
class OnboardingWizard:
    ...
    def enter_credentials(
        ...
        if not validation['valid']:
            logger.warning(f"Invalid credentials for {state.selected_broker.value}: {validation['errors']}")
            return validation
    ...
    def _validate_credentials_format(
        ...
        if broker_type == BrokerType.HYPERLIQUID:
            ...
            if not secret_key.startswith('0x') and len(secret_key) != 64:
                return {
                    "valid": False,
                    "errors": ["Secret key must be valid Ethereum private key (64 hex chars or 0x-prefixed)"],
                }
