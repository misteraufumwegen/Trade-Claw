...
class CorrelationMatrix(BaseModel):
    ...
    avg_correlation: float = Field(
        ...,
        description="Average correlation across all pairs"
    )
    high_correlation_pairs: int = Field(
        ...,
        description="Number of asset pairs above threshold"
    )
...
class CorrelationResponse(BaseModel):
    ...
    correlation_matrix: Dict[str, float] = Field(
        ...,
        description="All pairwise correlations"
    )
    ...
    trade_eligible: bool = Field(
        ...,
        description="Is trade eligible based on correlation?"
    )
...
class TradeCorrelationScore(BaseModel):
    ...
    score: float = Field(
        ...,
        description="Correlation score (0-100)",
        ge=0.0,
        le=100.0
    )
    avg_correlation: float = Field(..., description="Average correlation")
...
class AvailableAssetsResponse(BaseModel):
    ...
    assets: Dict[str, str] = Field(
        ...,
        description="Symbol -> Name mapping"
    )
